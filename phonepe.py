...
if selected == "About":
    ...
    col1.image(Image.open("C:\\Users\\mahes\\OneDrive\\Desktop\\MY PRACTICE\\pulse-2.jpg") ,width=400)
    with col1:
        ...
        st.markdown(f'''<a href={url1}><button style="background-color:#8a2be2;"><border style="display:inline-block;padding:0.5em 1em;border-radius:3px;border-color:none"><text style="color:#f2f3f4;">DOWNLOAD THE APP</button></a>''',unsafe_allow_html=True)
...
if selected == "Home":
    ...
    with col2:
        ...
        st.markdown(f'''<a href={url2}><button style="background-color:#8a2be2;"><border style="display:inline-block;padding:0.5em 1em;border-radius:3px;"><text style="color:#f2f3f4;">{text}</button></a>''',unsafe_allow_html=True)
...
if selected == "Insights":
    ...
    if option_ques == questions[0]:
        ...
        ex5 = dff.head(10)
        st.subheader(':violet[TOP 10 TRANSACTIONS STATES  TRANSACTIONS OF PHONEPE]')
        ...
        df_agg5 = df_top_trans_pin.groupby(['Pincodes']).agg({'Registered_users':sum}) 
